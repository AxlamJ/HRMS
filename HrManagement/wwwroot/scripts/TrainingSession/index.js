...
function renderTrainingStructure(data) {
  ...
  ${category.TrainingSubCategories?.length ? `
    <ul class="list-group ps-3 mb-3 collapse subcategory-list" id="subcat-${category.CategoryId}" role="group" aria-label="Subcategories">
      ${category.TrainingSubCategories.map(sub => `
...
`).join('')}
    ...
    if (firstCategory) {
        console.log(firstCategory);
        openLessonDetail(firstCategory.CategoryId, firstCategory.CategoryType, firstCategory.CategoryTitle)
    }
}
