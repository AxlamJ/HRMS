...
HRMSUtil.onDOMContentLoaded(function () {
    ...
    var ddTrainings = document.querySelector('#Trainings');
    $(ddTrainings).empty();
    var lstTraining = (DropDownsData.Trainings || []);
    var option = new Option();
    ddTrainings.appendChild(option);
    //var option = new Option("All", -1, false, false);
    //ddDepartments.appendChild(option);
    lstTraining.forEach(function (item) {

        var option = new Option(item.Title, item.TrainingId, false, false);
        ddTrainings.appendChild(option);
    });
...
$("#add-news-feed").on("click", function () {
    var form = document.querySelector("#add-news-form");
        ...
        var url = sitePath + 'api/AssignTrainingAPI/UpsertAssignTraining';
            ...
            if (response.StatusCode == 200) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href = "/AssignTraining/PermissionsByDepratment";
                    }
                });
            }
            else {
