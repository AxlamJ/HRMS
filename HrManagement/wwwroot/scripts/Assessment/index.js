...
function renderTrainingStructure(data) {
  ...
  <div class="ps-3 pt-3">
    <ul class="list-group">
     ${structure.Categories
            .filter(category => category.CategoryType === "3" || category.CategoryType=='2')
                .map(category => `
  ...
  ${category.TrainingSubCategories?.length ? `
    <ul class="list-group ps-2 mb-3 collapse subcategory-list" id="subcat-${category.CategoryId}" role="group" aria-label="Subcategories">
      ${category.TrainingSubCategories
                        .filter(sub => sub.CategoryType === "3")
                        .map(sub => `
        <li class="list-group-item mb-2 rounded shadow-sm border-0 d-flex justify-content-between align-items-center gap-2 category-item" role="listitem" onclick="openLessonDetail(${sub.CategoryId},${sub.CategoryType},'${sub.CategoryTitle}')" tabindex="0">
...
`).join('')}
    ...
    const firstCategory = data.Structures
        ...
        .flat()
        .find(cat => cat.CategoryType == "3");
    console.log(firstCategory);

    let LessonId = $("#lessonId").val();
    let title = $("#title").val();
    openLessonDetail(LessonId, "3", title);



    //if (firstCategory) {
    //    $('#comments-section').empty();
    //    $('#comments-section').append(`<button type="button" class="btn btn-primary" onclick="WriteComments(${firstCategory.CategoryId})" ><i class="bi bi-send-fill"></i></button>`);

    //    loadComments(firstCategory.CategoryId);
    //    $('#lessonTitle').empty();
    //    $('#lessonTitle').append(`<h3>${firstCategory.CategoryTitle}</h3>`);
    //    $("#StartQuiz").empty();
    //    $("#StartQuiz").append(`
    //     <button onclick="startQuiz(${firstCategory.CategoryId})" class="btn btn-lg btn-primary px-5 py-3">
    //        Start Quiz
    //    </button>
    //    `);
    //}
...
function openLessonDetail(id, Type, title) {
    ...
    if (Type == '3') {
        ...
        $('#lessonTitle').empty();
        $('#lessonTitle').append(`<h3>${title}</h3>`);
        $('#comments-section').empty();
        ...
        $("#StartQuiz").empty();
        $("#StartQuiz").append(`
         <button onclick="startQuiz(${id})" class="btn btn-lg btn-primary px-5 py-3">
            Start Quiz
        </button>
        `);
    }
...
function getLessonVideo(id, videoTitle) {
        ...
        Ajax.post(url, null, function (resp) {
            if (resp.StatusCode == 200) {
                ...
                $('#videoDesc').append(`<h3>${resp.Data.title}</h3>`);
                 playVideo(resp);
            }
            else {
                Swal.fire({
