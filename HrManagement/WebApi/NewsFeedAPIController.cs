...
namespace HrManagement.WebApi
{
    ...
    [ApiController]
    public class NewsFeedAPIController : ControllerBase
    {

        private readonly DataContext _context;
        private readonly IWebHostEnvironment _webHostEnvironment;

        public NewsFeedAPIController(DataContext context, IWebHostEnvironment webHostEnvironment)
        {
            _context = context;
        ...
        [HttpPost("UpsertNewsFeed")]
        public async Task<IActionResult> UpsertNewsFeed(NewsFeed news)
        {
            try
            {
                ...
                if (news != null)
                {
                    if (news.Id == null)
                    {
                        var InsertQuery = @"INSERT INTO NewsFeed 
                                            (
                                                 NewsTitle 
                                                ,Content
                                                ,ImageUrl
                        ...
                        news.CreatedById = loggedinUserId;
                        news.CreatedBy = loggedinUserFirstName + " " + loggedinUserLastName;
                        news.CreatedDate = DateTime.UtcNow;
                        news.ModifiedById = loggedinUserId;
                        news.ModifiedBy = loggedinUserFirstName + " " + loggedinUserLastName;
                        news.ModifiedDate = DateTime.UtcNow;
                        news.IsActive = true;
                        using var connection = _context.CreateConnection();
                        connection.Open();
                        await connection.ExecuteAsync(InsertQuery, news);
                        connection.Close();
                    ...
                    else
                    {
                        ...
                        var UpdateQuery = @"UPDATE NewsFeed SET 
